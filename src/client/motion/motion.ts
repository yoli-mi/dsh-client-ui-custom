...
export function installConversationEntrance(options: MotionEngineOptions): MotionEngine {
  ...
  const clearMarked = (): void => {
    ...
    for (const row of document.querySelectorAll<HTMLElement>(`.${SELECT_ANIMATION_CLASS}`)) {
  ...
  function scanTreeBoot(): void {
    ...
    if (selected !== undefined) playSelectAnimation(selected)
  }
  ...
  function flush(): void {
    ...
    for (const batch of fresh) {
      ...
      for (const row of batch.rows) {
        if (isTreeItem(row) && row.getAttribute('aria-selected') === 'true') playSelectAnimation(row)
      }
    }
    pending = []
  }
   ...
   */
  function playSelectAnimation(row: HTMLElement): void {
    ...
    if (state.transcript || state.sidebar) {
      for (const batch of batches) {
        ...
        if (batch.rows[0] !== undefined && isTreeItem(batch.rows[0])) {
          ...
          applyBatch(batch.rows, batch.load)
          for (const row of batch.rows) {
            if (row.getAttribute('aria-selected') === 'true') playSelectAnimation(row)
          }
          continue
        }
  ...
  function classEntrance(el: HTMLElement, cls: string): void {
