...
export type ShortcutAction =
  | 'newConversation' | 'switchModel' | 'cycleThinking'
  | 'sendMessage' | 'newline' | 'usagePanel'
 ...
 */
export function parseKeyCombo(spec: string | undefined): KeyCombo | null {
  ...
  for (const token of tokens) {
    if (MODIFIER_TOKENS.has(token)) { mod = true; continue }
    if (ALT_TOKENS.has(token)) { alt = true; continue }
    if (SHIFT_TOKENS.has(token)) { shift = true; continue }
    if (keyToken !== undefined) return null // two non-modifier keys
    keyToken = token
  }
  if (keyToken === undefined) return null // modifiers only, no key
  ...
  if (named !== undefined) return { mod, alt, shift, key: named }
  if (/^f([1-9]|1[0-9]|2[0-4])$/.test(keyToken)) return { mod, alt, shift, key: keyToken.toUpperCase() }
  return null
}
...
): Readonly<Record<ShortcutAction, KeyCombo | null>> {
  return {
...
export function comboEnabled(combo: KeyCombo | null): combo is KeyCombo {
  return combo !== null
}
 ...
 */
export function keyToToken(eventKey: string): string | null {
 ...
 */
export function composerRemapDecision(
  sendCombo: KeyCombo | null,
  newlineCombo: KeyCombo | null,
