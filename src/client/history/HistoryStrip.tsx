...
import { buildTurns, currentTurnKey, jumpToTurn, mergeVisibleTurns } from './turns.ts'
...
export type HistoryStripProps =
  ...
  & PropsLocale<'history'>
  & InjectFace<HistoryStripInjected>

/** Bar width % by distance from the hovered bar (the wave silhouette). */
const WAVE_WIDTH: Record<number, number> = { 0: 100, 1: 62, 2: 40, 3: 24 }
 ...
 */
export function HistoryStrip({ useSession, loadOlder, sessionId, useHistoryLimit, useHistoryPosition, usePinnedTurns, t }: HistoryStripProps) {
  ...
  const allTurns = useMemo(
    () => (blank || position === 'off') ? [] : buildTurns(chat),
    [chat, blank, position],
  )
  ...
  return (
    ...
    >
      {hovered !== null && createPortal(
        (
          <div
            className={`${css.tooltip} ${position === 'left' ? css.tooltipLeft : ''}`}
            style={{
            ...
            role="tooltip"
          >
            <span className={css.tooltipDot} aria-hidden />
            <span className={css.tooltipText}>
              {turns[hovered]?.question || translator('noText')}
            </span>
          </div>
        ),
