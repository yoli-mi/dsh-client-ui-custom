...
- **修改了通用设置项** —— 在「设置 → 通用」里新增了浮动历史条（位置、数量）和用户消息 Markdown 渲染两个开关，并且对历史条做了特别的使用优化（可以将指定对话在历史条位置悬挂）；
- **修改了插件项** —— 在「设置 → 插件」里新增了「插件市场」tab；
- **新增了三个设置页** —— 「外观」「快捷键」「用量统计」。

所以功能默认关闭，不配置时保持与原生界面一致，全程零 shell 改动。
...
| 设置 → 快捷键 | 新增页面 | 自定义快捷键，包括新建对话、切换模型、思考强度、模型直达等 |
