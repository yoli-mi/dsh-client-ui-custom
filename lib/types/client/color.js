 ...
 */ export function harmonySwatches(hex) {
 ...
 */
export function dominantColorFromRgba(data) {
...
const INSPIRATION_HUES = [0, 10, 20, 32, 46, 62, 82, 104, 124, 148, 172, 196, 220, 244, 264, 286, 306, 328];
const pick = (rng, options) => options[Math.min(options.length - 1, Math.floor(rng() * options.length))] ?? options[0] ?? undefined;
 ...
 */
export function randomInspirationConfig(rng = Math.random) {
    ...
    const glass = pick(rng, ['light', 'frosted', 'mica']);
